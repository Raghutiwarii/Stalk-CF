...
function Show(response){
    if(response['status'] == 'OK'){
        ...
        for(var i = 800; i<=3500; i+=100){
            if(arr1[i].length>0){
            var lett = i+"R";
            document.getElementById('accordion').innerHTML += "<table  class='table table-hover'><thead><tr><th style='text-align: center;' scope='col' >" + i +" Problem || Solved - "+ arr[i].length +"</th></tr></thead><tbody id='"+ lett +"'></tbody></table>";
